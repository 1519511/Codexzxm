...
function normalizeMcpToolResult(result) {
  ...
  return {
    ...
    meta: result?._meta === undefined ? null : structuredClone(result._meta),
  };
}
...
export class CodexPublicContextExecutor {
  ...
  async recover({ reason = "manual" } = {}) {
    ...
    const initialized = await this.#ensureStarted();
    return {
      status: "ok",
      recovered: true,
  ...
  async mcpToolCall({ server, tool, cwd = this.#defaultCwd, arguments: args = {}, meta = null, timeoutMs = 60_000, expectedGeneration = null }) {
    const effectiveCwd = path.resolve(cwd);
    ...
    const result = await this.#request("mcpServer/tool/call", params, {
      timeoutMs: Number.isInteger(timeoutMs) ? Math.min(120_000, Math.max(1_000, timeoutMs)) : 60_000,
      expectedGeneration,
    });
