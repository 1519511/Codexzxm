...
import { ACCEPTED_CODEX_VERSIONS, acceptedCodexVersionsFor, isCodexVersionAccepted } from "./codex-version-policy.mjs";
...
export { ACCEPTED_CODEX_VERSIONS, acceptedCodexVersionsFor };
...
export class CodexAuthorityExecutor {
  ...
  async validate() {
    ...
    if (!isCodexVersionAccepted(this.#codexVersion, { acceptedVersions: [...this.#acceptedCodexVersions] })) {
      throw new Error(
        `unsupported Codex CLI version for Codexzxm direct-profile authority: ${this.#codexVersion}. ` +
        `Accepted exact builds: ${[...this.#acceptedCodexVersions].join(", ")}. ` +
        "Stable Windows patch releases within an accepted minor line are allowed automatically; cross-minor or prerelease builds require explicit CODEXZXM_EXTRA_CODEX_VERSIONS acceptance."
      );
    }
