...
const testCwd = process.env.CODEXZXM_TEST_CWD ?? process.env.CODEXLESS_TEST_CWD;
...
function createIsolatedPublicTestEnv(extra = {}) {
  ...
  Object.assign(env, {
    ...
    CODEXZXM_PRIVATE_WORKBENCH: "0",
    ...(testCwd ? { CODEXZXM_DEFAULT_CWD: testCwd } : {}),
    ...extra,
  });
  return env;
}

assert.equal(PUBLIC_SURFACE_VERSION, "codexzxm-compat-v1");
...
const client = new Client({ name: "codexzxm-public-contract", version: "0.1.0" });
...
transport.stderr?.on("data", (chunk) => process.stderr.write(`[codexzxm] ${chunk}`));
...
async function waitForHttpHealth() {
  ...
  while (Date.now() < deadline) {
    if (httpChild.exitCode !== null) {
      throw new Error(`Codexzxm HTTP exited early (${httpChild.exitCode}): ${httpStderr}`);
    }
    try {
  ...
  throw new Error(`Codexzxm HTTP did not become healthy: ${String(lastError ?? "timeout")}\n${httpStderr}`);
}
...
try {
  ...
  assert.equal(health.service, "codexzxm");
  ...
  const httpClient = new Client({ name: "codexzxm-public-contract-http", version: "0.1.0" });
